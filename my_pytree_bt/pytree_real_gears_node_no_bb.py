...
BASE_WAYPOINT_OBJECTS_STATUS = [False] * len(BASE_WAYPOINT_OBJECTS)
...
class CheckVisitedWaypoint(py_trees.behaviour.Behaviour):
    def __init__(self, name, waypoint, waypoint_ind):
    ...
    def setup(self, **kwargs):
        ...
        self.wp_pstat_ublisher = self.node.create_publisher(
        ...
        # self.dist_to_target_subscriber = self.node.create_subscription(
        #     Float32, "/dist_to_target", self.dist_sub_callback, 10
        # )
    ...
    def update(self):
        self.wp_pstat_ublisher.publish(String(data=str(BASE_WAYPOINT_OBJECTS_STATUS)))
        ...
        if finished_mission or self.is_aruco_finish_mission():
            self.node.get_logger().info(f"WP {self.wp_ind+1}: >>> SUCCESS!!!")
            BASE_WAYPOINT_OBJECTS_STATUS[self.wp_ind] = True
        ...
        is_visited = BASE_WAYPOINT_OBJECTS_STATUS[self.wp_ind]

        self.node.get_logger().info(
            f"Update, Is WP {self.wp_ind +1} Visited? {is_visited}"
        )
...
class Stop(py_trees.behaviour.Behaviour):
    def __init__(self, name, flick=False):
...
class CheckSideWall(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        self.node.get_logger().info(f"Near Left Wall Zed: {self.is_near_left_wall}")
        self.node.get_logger().info(f"Near Right Wall Zed: {self.is_near_right_wall}")

        self.node.get_logger().info(
            f"Near Left Wall Lidar: {self.is_lidar_near_left_wall}"
        )
        self.node.get_logger().info(
            f"Near Right Wall Lidar: {self.is_lidar_near_right_wall}"
        )

        if self.is_near_wall or self.is_near_wall_zed:
            # self.cmd_vel_publisher_.publish(self.cmd_vel_msg)
...
class GoToWayPoint(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        self.finished_mission = finished_mission
        ...
        self.curr_waypoint_status_pub.publish(Bool(data=finished_mission))
...
def create_core_wp_subtree(waypoint_index, waypoint):
    ...
    guard_wp_checker = CheckVisitedWaypoint(
        name=f"Guard WP Check {waypoint_index}",
        waypoint=waypoint[waypoint_index - 1],
        waypoint_ind=waypoint_index,
    )
    ...
    stop_action_for_rotate = Stop(name=f"Stop Spin{waypoint_index}", flick=True)
